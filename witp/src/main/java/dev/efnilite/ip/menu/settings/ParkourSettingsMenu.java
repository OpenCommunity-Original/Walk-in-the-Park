...
public class ParkourSettingsMenu extends DynamicMenu {
    ...
    public ParkourSettingsMenu(ParkourOption... disabled) {
        ...
        registerMainItem(1, 8, (p, user) -> {
            ...
            List<String> values = Locales.getStringList(user.player, "%s.values".formatted(ParkourOption.SCHEMATICS.path));
    ...
    public void open(ParkourPlayer user) {
        display(user.player, new Menu(4, Locales.getString(user.player, "settings.name"))
            .distributeRowEvenly(0, 1, 2, 3)
            .item(27, Locales.getItem(user.player, "other.close").click(event -> Menus.SETTINGS.open(event.getPlayer())))
