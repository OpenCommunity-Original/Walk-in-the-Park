...
public class LangMenu {
    ...
    public void open(ParkourPlayer user) {
        ...
        PagedMenu style = new PagedMenu(3, Locales.getString(user.player, "settings.lang.name"));

        List<MenuItem> items = new ArrayList<>();
        for (String lang : Locales.locales.keySet()) {
            Item item = new Item(Material.PAPER, "<#238681><bold>" + Locales.getString(user.player, "name"));

            items.add(item.glowing(user.player.equals(lang)).click(event -> {
        ...
        style.displayRows(0, 1).addToDisplay(items)
                ...
                .item(22, Locales.getItem(user.player, "other.close").click(event -> Menus.SETTINGS.open(event.getPlayer())))
